...
def run_spark_job(spark):
    spark.sparkContext.setLogLevel("WARN") # disable the info logs and will just print the warnings
    ...
    # i think we want distinct_table and not agg_df here
    join_df = distinct_table.join(radio_code_df, 'disposition')

    join_df.printSchema()
    
    join_query = join_df.writeStream \
                       .queryName("join_query_write") \
                       .format("console") \
                       .start()
...
if __name__ == "__main__":
    ...
    logger.warn(spark.sparkContext.getConf().getAll())
